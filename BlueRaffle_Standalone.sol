 ...
 * @notice Manually defined interface for VRF v2.5 coordinator
 * @dev Uses uint256 for subscription IDs (not uint64 like v2)
 */
interface IVRFCoordinatorV2Plus {
    function requestRandomWords(
        bytes32 keyHash,
        uint256 subId,  // uint256 for v2.5 (v2 used uint64)
...
/**
 * @title BlueRaffle - Standalone Version
 * @notice Casino-style raffle for BlueBNB DeFi protocol
 * @dev Standalone version with manually defined VRF interface to avoid Chainlink dependency issues
 * @dev Supports VRF v2.5 with uint256 subscription IDs
 */
contract BlueRaffle is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        subscriptionId = _subscriptionId;  // uint256 - no casting needed
        keyHash = _keyHash;
     ...
     * @dev Must be public and check msg.sender is VRF coordinator
     */
    ...
    function closeRound() external whenNotPaused nonReentrant {
        ...
        uint256 requestId = vrfCoordinator.requestRandomWords(
            keyHash,
            subscriptionId,  // uint256 - no casting to uint64
