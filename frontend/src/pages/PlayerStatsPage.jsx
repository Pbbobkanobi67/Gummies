...
function PlayerStatsPage({ wallet, dice, progressive, raffle }) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // Fetch progressive history
        if (progressive.contract) {
    ...
    fetchHistory();
  }, [wallet.account, dice.contract, progressive.contract]);
  ...
  const progressiveStats = React.useMemo(() => {
    ...
    const totalSpent = totalRolls * 100; // Assuming 100 BLUE per ticket
  ...
  return (
    <div className="player-stats-page">
      ...
      <div className="card player-overview-card">
        ...
        <div className="player-info">
          ...
          <div className="player-balance">
            <span className="label">BLUE Balance:</span>
            <span className="balance-value">{parseFloat(blueBalance).toLocaleString()} BLUE</span>
          </div>
        </div>
      </div>
          ...
          {diceStats ? (
            <div className="stats-grid">
              ...
              <div className="stat-item full-width">
                <span className="stat-value">{diceStats.totalWagered.toLocaleString()} BLUE</span>
                <span className="stat-label">Total Wagered</span>
              </div>
              <div className="stat-item full-width">
                <span className={`stat-value ${diceStats.netProfit >= 0 ? 'win' : 'loss'}`}>
                  {diceStats.netProfit >= 0 ? '+' : ''}{diceStats.netProfit.toLocaleString()} BLUE
                </span>
        ...
        <div className="card stats-card progressive-stats">
          ...
          {progressiveStats ? (
            <div className="stats-grid">
              ...
              <div className="stat-item full-width">
                <span className="stat-value">{progressiveStats.totalSpent.toLocaleString()} BLUE</span>
                <span className="stat-label">Total Spent</span>
              </div>
              <div className="stat-item full-width">
                <span className={`stat-value ${progressiveStats.netProfit >= 0 ? 'win' : 'loss'}`}>
                  {progressiveStats.netProfit >= 0 ? '+' : ''}{progressiveStats.netProfit.toLocaleString()} BLUE
                </span>
                <span className="stat-label">Net Profit/Loss</span>
              </div>
            </div>
      ...
      <div className="card history-card">
        <h3>Recent Classic Dice Bets</h3>
        {loading ? (
              ...
              <tbody>
                {diceHistory.map((bet, index) => (
                  <tr key={index}>
                    ...
                    <td>{parseFloat(bet.amount).toLocaleString()} BLUE</td>
                    <td>{bet.rolledNumber || '-'}</td>
                    <td>{bet.status === 1 ? `${parseFloat(bet.payout).toLocaleString()} BLUE` : '-'}</td>
                    <td>
                  ...
                  <tr key={index} className={roll.matches === 4 ? 'jackpot-row' : ''}>
                    ...
                    <td className={parseFloat(roll.payout) > 0 ? 'payout-win' : ''}>
                      {parseFloat(roll.payout) > 0
                        ? `${parseFloat(roll.payout).toLocaleString()} BLUE`
                        : '-'
                      }
                    </td>
                  </tr>
